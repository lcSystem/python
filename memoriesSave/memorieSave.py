...
class TextEditorApp:
    ...
    def load_selected_file(self, event):
        ...
        if selected_file:
            self.current_file = selected_file  # Track the current file
    ...
    def save_file(self):
        ...
        if filename:
            ...
            if self.current_file and new_filename == self.current_file:
                filepath = os.path.join("notas", self.current_file)
    ...
    def save_as_new_file(self, content, filename=None):
        ...
        if filename:
            ...
            self.load_file_list()
            self.name_entry.delete(0, tk.END)
            self.name_entry.insert(0, filename)  # Keep the filename in the entry box
