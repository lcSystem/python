...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ax.set_title('Diagrama de Puntos y Líneas')
    ...
    def actualizar_grafico(self):
        self.ax.clear()
        self.ax.plot(range(1, len(self.datos) + 1), self.datos, marker='o', linestyle='-')
        self.ax.set_xlabel('Índice')
        self.ax.set_ylabel('Valor')
        self.ax.set_title('Diagrama de Puntos y Líneas')
        for i, valor in enumerate(self.datos):
            if valor <= 1.5:
                self.ax.scatter(i + 1, valor, color='red')
            elif 1.5 < valor <= 2:
                self.ax.scatter(i + 1, valor, color='orange')
            elif 2 < valor <= 3.5:
                self.ax.scatter(i + 1, valor, color='blue')
            else:
                self.ax.scatter(i + 1, valor, color='green')
        self.canvas.draw()
